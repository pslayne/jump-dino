...
#endereço
screenStart: .word 0x10040000 #a tela começa na heap
...
#desenha o cenário
	...
	lw $t2, fundo #t2 recebe a cor do fundo
	
	desenhaLinha:
		sw $t2, ($t3) #faz o store da cor de fundo em t3
	...
	desenhaChao:
		...
		desenhaChaoLinha:
			sw $t2, ($t3) #faz o store da cor do cacto em t3
			addi $t3, $t3, 4 #incrementa o endereço
...
#pinta todos o cenário da cor do cacto quando o jogador perde
	...
	lw $t2, cacto #t2 recebe a cor do cacto
	
	desenhaLinha:
		sw $t2, ($t3) #faz o store da cor para o endereço em t3
		addi $t3, $t3, 4 #incrementa o endereço
		addi $t1, $t1, 4 #incrementa o contador
		SleepShorter
		blt $t1, $t0, desenhaLinha
...
apagaDino:
	...
	apagaCorpoVer:
		...
		li $t1, 0 # reset the horizontal counter
		addi $t2, $t2, 1
		blt $t2, $t7, apagaCorpoHor
		
		li $t4, 5 # maximum head width
		li $t5, 20 # how much to move back for each level with head
		blt $t2, $t6, apagaCorpoHor
		j apagaCauda
